...
le::IConCmd*			cmd_flashlight = nullptr;
...
// ------------------------------------------------------------------------------------ //
// Console command: flashlight
// ------------------------------------------------------------------------------------ //
void CMD_Flashlight( le::UInt32_t CountArguments, const char** Arguments )
{
	buttons[ BT_FLASHLIGHT ] = true;
}
...
Input::~Input()
{
	...
	g_consoleSystem->UnregisterCommand( "flashlight" );
}
...
void Input::Initialize()
{
	...
	cmd_flashlight = ( le::IConCmd* ) factory->Create( CONCMD_INTERFACE_VERSION );
	...
	cmd_flashlight->Initialize( "flashlight", "Flashlight enable/disable", CMD_Flashlight );
	...
	g_consoleSystem->RegisterCommand( cmd_flashlight );
}

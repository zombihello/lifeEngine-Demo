...
void Player::Update()
{
	...
	case CT_PHYSICS:
	{
		...
		if ( input->IsButtonDown( BT_MOVE_LEFT ) )			offsetDirection += camera->GetDirectionMove( le::CSM_LEFT );
		if ( input->IsButtonDown( BT_MOVE_RIGHT ) )			offsetDirection += camera->GetDirectionMove( le::CSM_RIGHT );
